...
export default function ResultsDetail(props: { item: BusinessResponse }) {
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.rating}>Rating: <Text style={{ color: props.item.rating > 4 ? '#3bc3' : '#ea3', fontWeight: 'bold' }}>{props.item.rating}</Text> ({props.item.review_count} reviews)</Text>
        </View>
    );
}
